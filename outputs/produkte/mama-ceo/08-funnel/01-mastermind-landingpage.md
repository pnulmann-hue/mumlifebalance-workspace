# Mama-CEO KI-Mastermind — Landingpage-Briefing

**Datum:** 2026-05-01
**Webinar-Titel (FINAL):** „In 90 Min: Dein Mama-Leben mit KI-Assistenten umkrempeln"
**Untertitel:** „Mental Load abgeben · Zeit fürs Business zurück gewinnen"
**Ziel:** Landingpage live ab Mi 13.5. — mind. 100 Mastermind-Anmeldungen bis Mi 20.5. 09:00
**Tech-Empfehlung:** **WordPress (mumlifebalance.ch/ki-mastermind)** — via `/wp` direkt deploybar, du hast Admin · Alternative: ThriveCart Free Page
**Anmeldung:** ManyChat-Keyword `MASTERMIND` als primärer CTA + AC-Formular als Fallback
**Live geht die Page:** Mittwoch 13.5. 10:00 (siehe `07-launch-kalender.md`)

---

## Strategie: Was diese Landingpage WOW macht

```
Hook        → benennt Mental Load + Schuld-Spirale (sofort Resonanz)
Painpoint   → spiegelt 5 typische Mompreneurin-Sätze (Identifikation)
Vision      → KI als Mitarbeiterin (ungewohnt, aber konkret)
Live-Demo   → „Ich zeige dir LIVE 30 Min Bildschirm" (Beweis)
Story       → Brandastic Schwester-Tod-Aha (Tiefe + Authentizität)
Live-Bonus  → 1 KI-Bot-Template als Geschenk nur Live (Knappheit)
CTA         → klare Anmeldung mit ManyChat MASTERMIND
```

**Brand-Bausteine:**
- Schweizer Du-Form, ss statt ß
- Patricias Original-Sätze (Brandastic-Frame): „KI-Mitarbeiter, die mir genau die Arbeit abnehmen, die wir als Networkerin gar nicht haben wollen"
- Schaufenster-Metapher (wenn passend)
- Aurachirurgie-Tiefe in der Story-Sektion (nicht im Pitch)
- Killer-Differenzierung: „Andere zeigen dir KI fürs Business. Andere zeigen dir KI für den Haushalt. **Mama-CEO verbindet beides.**"

---

## SEKTION 1 — HERO (über dem Fold)

**Eyebrow (kleiner Text oben):**
> 🔴 LIVE KOSTENLOS · Mi 20. Mai 2026 · 09:00 Uhr · 90 Min

**Headline (gross):**
> # In 90 Min: Dein Mama-Leben mit KI-Assistenten umkrempeln.

**Sub-Headline:**
> ## Mental Load abgeben · Zeit fürs Business zurück gewinnen.

**Hero-Body (3-4 Zeilen):**
> Du sagst dir täglich „Ich sollte unbedingt…" — und kommst trotzdem nie dazu? Du machst alles selbst, weil delegieren zu kompliziert ist? Du brennst aus, weil bei dir alles parallel läuft — Business, Haushalt, Kinder?
>
> **In diesem Live-Mastermind zeige ich dir, wie KI dich genau da entlastet — am Ende der 90 Min bist du raus aus der „Hab keine Zeit fürs Business"-Spirale.**

**CTA-Box (rechts oder unter Hero):**
> ✅ Mittwoch 20. Mai 2026, 09:00 Uhr
> ✅ 90 Min LIVE inkl. Bildschirm-Demo
> ✅ Aufzeichnung 48h verfügbar
> ✅ 100% kostenlos
>
> [👉 Schreib „MASTERMIND" auf Instagram] (verlinkt zu ManyChat)
> [oder Anmeldung via E-Mail] (AC-Formular Fallback)

**Hero-Visual:** Patricia-Foto authentisch (Setup mit Laptop + Kaffee, lächelnd) — aus `context/Shootingbilder/`

---

## SEKTION 2 — PAINPOINT-CHECKLISTE „Kommt dir bekannt vor?"

**Headline:**
> ## Kommt dir das bekannt vor?

**Body (mit Häkchen oder X-Symbolen, sehr persönlich):**
> ☑ Du startest deinen Tag mit dem Vorsatz „Heute mach ich endlich Content" — und am Abend ist nichts gepostet.
>
> ☑ Du hast 47 Apps, Tools und Listen — und trotzdem fühlst du dich, als würdest du alles im Kopf tragen.
>
> ☑ Du planst Wochen mit Kids, Haushalt, Kunden, Team-Calls, Schul-Termine — und fragst dich: *Warum mache ich eigentlich alles allein?*
>
> ☑ Du weisst, du müsstest delegieren — aber niemanden für 4'000 CHF/Monat einstellen können.
>
> ☑ Und du hast schon 3 Kurse gekauft, die dir „Productivity" versprochen haben. Geliefert haben sie dir mehr Druck.

**Aurachirurgie-Frame:**
> Das ist nicht dein Versagen. **Das ist die Schuld-Spirale**, in der jede Mompreneurin steckt — und die endet nicht mit einem weiteren To-Do-Tracker.

---

## SEKTION 3 — VISION „Stell dir vor…"

**Headline:**
> ## Stell dir vor, am Sonntag-Abend…

**Body (Tagebuch-Style, persönlich):**
> Du schreibst deinem Familien-Bot: *„Plan mir die Woche mit Schul-Terminen, Power-Zeiten und 2 Kunden-Calls."*
>
> 30 Sekunden später hast du:
> - einen vollständigen Wochenplan,
> - die passende Einkaufsliste,
> - 3 Caption-Vorschläge für deine Reels,
> - eine Pitch-Mail an die Kundin, die du seit 2 Wochen aufschiebst.
>
> Das alles ohne 1× Ctrl+C, ohne 1× „warte, was war nochmal die Schul-Adresse?", ohne 1× das schlechte Gewissen, dass du eigentlich noch 12 andere Dinge machen müsstest.
>
> **Genau das zeige ich dir Live am Mittwoch — Schritt für Schritt.**

---

## SEKTION 4 — „WAS DU IM 90-MIN-MASTERMIND LERNST"

**Headline:**
> ## Was du in diesen 90 Minuten mitnimmst

**3 Spalten (oder Liste mit Icons):**

> **🧠 Die Schuld-Spirale durchschauen**
> Warum „Ich sollte unbedingt" kein Charakterfehler ist — und wie du den Kreislauf in 1 Schritt unterbrichst (mit Daten-Anker aus 2026).

> **🤖 Dein erster KI-Mitarbeiter LIVE gebaut**
> Ich zeige dir 30 Min Bildschirm-Aufnahme: Custom GPT für Familien-Mental-Load · Brand-Voice-Bot für Reels-Hooks · Wie ich beide täglich nutze.

> **⚡ Der Wochenrhythmus, der nicht zerbricht wenn Kinder krank sind**
> Mein Energie-Wochenplan + die 4 Mama-Rollen — und warum 2 Power-Zeiten täglich reichen, wenn du sie ehrlich findest.

**Bonus-Box (auffällig, vielleicht Sticker-Style):**
> 🎁 **NUR LIVE-TEILNEHMER:** Du bekommst meine fertige Setup-Anleitung für den Familien-Bot zum Download — Custom-GPT-Vorlage + Prompt-Bibliothek + 5 fertige Wochenpläne.

---

## SEKTION 5 — AGENDA (was passiert genau)

**Headline:**
> ## Der Ablauf am Mi 20. Mai 09:00–10:30

**Timeline-Style:**
> **09:00 — 09:10** Story: Wie ich aus dem Hamsterrad rausgekommen bin (10 Min)
>
> **09:10 — 09:25** Schuld-Spirale entlarvt — und warum 58% der Solo-Unternehmer:innen mit KI 20+ Stunden im Monat sparen (15 Min, mit Daten)
>
> **09:25 — 09:55** ⭐ **DAS HERZSTÜCK:** Live-Demo Familien-Bot + Brand-Voice-Bot — direkt am Bildschirm. Du siehst, was rein geht, was raus kommt, wie du es selbst nachbaust. (30 Min)
>
> **09:55 — 10:10** Wie es nach dem Mastermind weitergeht — mit Mama-CEO als Möglichkeit (15 Min)
>
> **10:10 — 10:30** Q&A — du fragst, ich antworte (20 Min)

---

## SEKTION 6 — ÜBER PATRICIA (Brandastic-Story)

**Headline:**
> ## Wer das hier hält

**Body (kurze Bridge, persönlich):**
> Ich bin Patricia. Mompreneurin, Network-Coach, Mama von 2.
>
> Und ich war lange Zeit unglücklich. Gefangen im Hamsterrad. Mein Leben auf Standby.
>
> Bis meine Schwester mit 44 an Krebs verstarb — und mir die Augen geöffnet hat, dass ich nicht länger warten will. Eine Mutter darf ein eigenes Leben leben. Und sie darf sich Mitarbeiter holen — auch wenn diese Mitarbeiter heute keine Menschen sind, sondern KI.
>
> Heute baue ich seit 2 Jahren digitale Mitarbeiter, die mir genau die Arbeit abnehmen, die wir als Networkerin gar nicht haben wollen — den Adminkram. **Und auch in Haushaltsdingen — Mental Load lass grüssen.**
>
> Genau das gebe ich dir am Mittwoch weiter.

**Foto Patricia:** Authentisch, eher persönlich (z.B. mit Kindern oder allein im Garten — aus `context/Shootingbilder/`)

---

## SEKTION 7 — TROPHÄE (Live-Bonus & Replay)

**Headline:**
> ## Was du bekommst, wenn du LIVE dabei bist

**2-Spalten-Layout:**

> **🎁 Live-Geschenk (nur Mi 20.5.)**
> Setup-Anleitung Familien-Bot:
> - Custom-GPT-Vorlage (copy-paste-ready)
> - 12 Prompt-Beispiele
> - 5 fertige Wochenplan-Templates
> Wert: CHF 49 · für dich kostenlos.

> **📹 Replay (48h)**
> Falls du Mi 09:00 nicht kannst:
> - Aufzeichnung 48h verfügbar (Fr 22.5. 09:00 läuft sie aus)
> - Alle Demos in voller Länge
> - Kein Live-Bonus — der bleibt Live-Teilnehmern vorbehalten.

---

## SEKTION 8 — ANMELDUNG (Haupt-CTA)

**Headline:**
> ## Sichere dir deinen Platz — kostenlos.

**Anmelde-Block (mittig, gross):**
> 📅 **Mittwoch 20. Mai 2026, 09:00 Uhr**
> ⏱️ 90 Min Live + Q&A
> 💻 Zoom-Webinar (Link kommt per E-Mail)
> 🎁 Setup-Anleitung Familien-Bot als Live-Geschenk
> 💸 Kosten: 0 €
>
> [👉 Schreib „MASTERMIND" auf Instagram (mumlifebalance)]
> [oder: hier per E-Mail anmelden ↓]

**Fallback-Formular (AC-Embed):**
- Vorname (Pflicht)
- E-Mail (Pflicht)
- Frage „Was ist dein grösster Mental-Load-Punkt gerade?" (optional, freier Text — fliesst in Q&A-Vorbereitung)
- Submit-Button: „Ja, ich will dabei sein"

**Tag in AC nach Anmeldung:** `mama-ceo-mastermind-anmeldung`

---

## SEKTION 9 — FAQ

**Headline:**
> ## Häufige Fragen

**6 Fragen (Akkordeon-Style):**

> **„Ich kann am Mi 09:00 nicht live dabei sein — lohnt es sich trotzdem?"**
> Ja — die Aufzeichnung ist 48h verfügbar. Du verpasst nur das Live-Geschenk (Familien-Bot-Setup-Anleitung). Alle Demos siehst du in voller Länge.

> **„Bin ich technisch fit genug? Ich bin nicht so KI-affin."**
> Genau dafür ist das Mastermind. Ich zeige dir live, wie ich vom „Ich kann das nicht" zur täglichen KI-Nutzung gekommen bin. Wenn du Instagram bedienen kannst, kannst du auch das.

> **„Werde ich am Ende ein Produkt verkauft bekommen?"**
> Ja — am Ende stelle ich dir Mama-CEO vor (mein 8-Wochen-Programm), du entscheidest komplett selbst, ob das für dich passt. Den ersten 90 Min sind 100% Mehrwert ohne Pitch.

> **„Wie kann ich Patricia eine Frage stellen?"**
> Im Live-Q&A (20 Min) oder direkt per Instagram-DM nach dem Mastermind. Beantworte ich persönlich.

> **„Wann genau startet das Webinar — auch in Deutschland 09:00?"**
> Ja, 09:00 Uhr Schweizer Zeit = 09:00 Uhr deutsche Zeit (gleiche Zeitzone CEST/MESZ).

> **„Brauche ich Zoom?"**
> Du brauchst nur einen Link, den du per E-Mail bekommst. Du kannst entweder die Zoom-App nutzen oder direkt im Browser teilnehmen — beides funktioniert.

---

## SEKTION 10 — FOOTER (kompakt)

> KI-Mastermind „Dein erster KI-Mitarbeiter in 90 Min" — Mum Life Balance · Patricia Nüllmann · Schweiz
>
> [Impressum] · [Datenschutz] · [Kontakt]

---

## Layout & Design-Hinweise

**Brand-Farben:**
- Background: Creme `#f1ecdd` (Hauptfarbe)
- Akzent: dunkler Schriftton (Patricia-Brand)
- CTA-Button: kontrastreich (z.B. dunkel-petrol oder warmes Rot)

**Schriften:**
- Headlines: Philosopher (Patricia-Standard)
- Body: Source Sans 3 (Patricia-Standard)

**Mobile-Optimierung:**
- Hero-Headline + CTA müssen über dem Fold auf Mobile sichtbar sein
- AC-Formular touch-friendly
- Bilder lazy-loaded für schnelle Ladezeit

**Bilder (aus `context/Shootingbilder/`):**
- Hero: Patricia mit Laptop + Kaffee + Lächeln (warm)
- About: Patricia mit Kindern oder allein in der Natur (persönlich)
- Bonus-Box: Screenshot-Mockup vom Familien-Bot in Action

---

## Anschluss

**Was kommt jetzt:**
1. **Patricia-Check** dieser Inhalt — passen Texte zur Voice? Bonus stimmig?
2. Tech-Entscheid: WordPress (`/wp` direkt deploy als Draft) oder ThriveCart Free Page?
3. Bei WP: ich rufe `/wp` und deploye `mumlifebalance.ch/ki-mastermind` als Draft (Patricia prüft + publiziert)
4. AC-Formular setup (Tag `mama-ceo-mastermind-anmeldung`)
5. ManyChat-Keyword `MASTERMIND` Flow scharfstellen
6. Erste Stories-Bumps zur Page ab Mi 13.5.

**Verknüpfungen:**
- Mastermind-Konzept: `outputs/produkte/mama-ceo/08-funnel/00-mastermind-konzept.md`
- Launch-Kalender: `outputs/produkte/mama-ceo/07-launch-kalender.md`
- Briefing: `outputs/produkte/mama-ceo/00-briefing-FINAL.md`
- Notion-Aufgabe „Mastermind-Anmelde-Page LIVE setzen" — Mi 13.5. 10:00
