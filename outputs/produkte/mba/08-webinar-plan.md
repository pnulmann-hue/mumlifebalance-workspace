---
tags: [produkt, webinar, launch, mba]
---

# MBA Launch-Webinar — Konzept & Pitch-Plan

> **Produkt:** Mum Business Academy (MBA) — dein Funnel-Endprodukt · Bundle aus 3 Kursen + Umsetzerinnen-Calls + Community
> **Termin:** 🗓️ **Mi 24.6.2026, 09:00** (Tag 0 = Cart-Open) · Pioneer bis Sa 27.6. 23:59 · Cart-Close So 5.7. 23:59
> **Webinar-Typ:** Live-Launch-Event → danach Evergreen in die MBA-Pitch-Automation (siehe [[umbau-plan]])
> **Zielgruppe:** KALT — über Meta-Ads (NWM-Mamas DACH, 30–45, „dabei aber nicht durchgebrochen") · Webinar selbst = der 0€-Leadmagnet
> **Angle:** Emotion + „Schon 1 Jahr im Network und immer noch kein richtiges Einkommen?"
> **Prinzip:** Zeig **WAS** möglich ist (deine Realität + Live-Demo), nicht das ganze **WIE** — das WIE ist die MBA.
> **Preis im Pitch:** Pioneer CHF 990 · Listenpreis CHF 1347 · bis zu 6 Raten
> Erstellt: 2026-06-04 · Status: Konzept zum Freigeben · richtet sich auf [[launch-kalender-juli-2026]] aus

---

## 0. Auf einen Blick

| | |
|---|---|
| **Ziel des Webinars** | Kalte Ads-Leads emotional abholen → in die MBA (990) verkaufen |
| **Länge** | ~90 Min (75 Min Inhalt + Pitch, 15 Min Q&A) |
| **Kern-Transformation** | Weg von der Produkt-/Network-Abhängigkeit → hin zu deinem eigenen, planbaren Einkommen mit System |
| **Die 3 Säulen (= 3 MBA-Kurse)** | 1. Sichtbarkeit & eigenes Thema (Insta-Kundenmaschine) · 2. Eigenes Angebot / Hybrideinkommen (Digitale Produktwelt) · 3. Zeit + KI (Mama-CEO) |
| **Differenzierer** | Umsetzerinnen-Calls (2×/Monat: Expertinnen + Patricia) + Community (Circle) |
| **Conversion-Mechanik** | Cart-Open am Webinar-Ende · Fast-Action-Bonus · Pioneer-Preis nur erste Runde |

---

## 1. Strategie-Fundament (INTERN — nicht auf Slides)

### Das Emotion-Prinzip (dein Ralf-Schmitz-Learning)
Emotionen verkaufen. Das Webinar zeigt **WAS** möglich ist — deine echte Realität, eine Live-Demo, ein Gefühl von „das könnte auch mein Leben sein". Es zeigt **nicht** das komplette **WIE** (Schritt-für-Schritt), denn das ist genau der Wert, den die MBA liefert. Wer am Ende das vollständige System will, kauft. Wer nur inspiriert wurde, hat trotzdem etwas mitgenommen — und kommt über den Evergreen oder die Ads zurück.

→ **Praktisch:** In jeder Säulen-Phase nennst du das Ergebnis und zeigst einen Ausschnitt (z.B. den Companion live), aber du erklärst nicht die ganze Methode. „Ich zeig dir, dass es geht — wie genau, das bauen wir zusammen in der Academy."

### Die emotionale Kern-Geschichte (dein USP gegenüber jeder Bali-Coachin)
Dein stärkster, ehrlichster Anker für dieses Webinar ist **die Abhängigkeits-Falle**: Du wolltest nicht mehr davon abhängig sein, wie viele Teampartnerinnen du findest und was die tun. Das ist ein Gefühl, das deine kalte Zielgruppe sofort im Bauch spürt — weil sie genau da feststeckt. Daraus entsteht die ganze Verschiebung: **weg vom Produkt, hin zur Transformation; weg von fremder Abhängigkeit, hin zu deinem eigenen Standbein.**

### Die 3-Säulen-Logik → das rechtfertigt das Bundle
Eine Mama braucht alle drei, sonst bleibt sie hängen:
1. **Sichtbarkeit & eigenes Thema** — ohne das hat sie viele Follower, aber null Anfragen → *Insta-Kundenmaschine*
2. **Ein eigenes Angebot / zweites Standbein** — damit sie nicht nur Provision verdient, sondern eigenen Umsatz → *Digitale Produktwelt*
3. **Zeit + KI-System** — damit das alles neben Familie überhaupt läuft → *Mama-CEO*

→ Genau deshalb ist die MBA ein **Bundle**, kein Einzelkurs: Ein Kurs allein löst nur ein Drittel. Die Umsetzerinnen-Calls halten alles zusammen und sorgen dafür, dass sie's wirklich umsetzt (Anti-„Kurs-gekauft-nichts-getan").

### Cold-Traffic-Implikationen (weil über Meta-Ads)
- **Mehr Story, mehr Vertrauen, später pitchen.** Eine fremde Frau braucht erst das Gefühl „die ist echt, die meint's ehrlich", bevor 990 fällt.
- **Niedrigere Einstiegsschwelle:** Zahlplan + Fast-Action-Bonus + echte Garantie sind bei kalt fast Pflicht (siehe Sektion 6).
- 🚨 **Meta-Income-Claim-Warnung:** In der **Ad** (nicht im Webinar selbst) darfst du **keine konkreten Einkommens-Versprechen** machen („verdiene 2x vierstellig"). Meta sperrt das. Die Ad verkauft die **Transformation/Freiheit** („dein eigenes Standbein, unabhängig"), die Einkommens-Story erzählst du erst **im** Webinar als deine persönliche Realität. → Verknüpft mit [[reference_ki-webinar-lp-technik]] + [[feedback_KRITISCH-ad-link-pre-launch-test]] (Link vor Aktivierung testen!).

### Warum dieser Pitch anders ist als das Mama-CEO-Webinar (333)
Mama-CEO war ein Mid-Kurs mit Demo-Schwerpunkt. Hier verkaufst du dein **Premium-Endprodukt**. Das heisst: stärkerer Wert-Stack, die Community als emotionaler Differenzierer, eine ehrliche Preis-Rechtfertigung (3 Kurse + laufende Begleitung) und mehr Risk-Reversal, weil die Summe grösser ist.

---

## 2. Webinar-Titel (festgelegt) + Meta-safe Ad-Variante

**🎯 Webinar-Titel (Patricia, festgelegt):**
> „Schon 1 Jahr im Network und immer noch kein richtiges Einkommen? Ich zeige dir, wie ich als 4-fach-Mama ein 2x vierstelliges Einkommen aufgebaut habe."

Trifft den Painpoint direkt im ersten Halbsatz, dann sofort Beweis-Versprechen. Perfekt für **Webinar selbst + E-Mail-Einladung + organische Stories**.

> 🚨 **Achtung Meta-Ad-Zielseite:** Der Income-Teil („2x vierstelliges Einkommen") ist deine echte Realität und im Webinar/per Mail erlaubt — aber **Meta prüft auch die Anmelde-Landingpage**, auf die die kalte Ad führt. Steht die Einkommens-Zahl dort prominent, riskierst du Ad-Ablehnung. Deshalb zwei Ebenen:

**Für die Ad + Anmelde-LP (Meta-safe, ohne Income-Claim):**
> „Schon eine Weile im Network — und trotzdem fragt kaum jemand an? Im kostenlosen Live-Webinar zeig ich dir als 4-fach-Mama, wie du dir ein eigenes, planbares Standbein aufbaust — ohne ständig neue Leute zu brauchen."

→ Die Einkommens-Zahl erzählst du **im Webinar** als deine Geschichte (Slide 6), nicht in der Ad.

---

## 3. Dramaturgie — Phasen-Übersicht (~90 Min)

| Phase | Min | Slides | Funktion |
|---|---|---|---|
| **1. STORY / EMOTION** | 0–20 | 1–6 | Vertrauen + Abhängigkeits-Frust + Wendepunkt (kalt = länger) |
| **2. REFRAME** | 20–35 | 7–10 | Die Falle benennen → der Shift (Produkt→Transformation) → die 3 Säulen |
| **3. MÖGLICHKEIT ZEIGEN** | 35–55 | 11–14 | WAS möglich ist: Companion-Demo + die 3 Säulen in Aktion |
| **4. PITCH MBA** | 55–73 | 15–21 | Die Academy: 3 Kurse + Community + Boni |
| **5. PREIS + RISK + ANTIKUNDEN** | 73–84 | 22–25 | Wert-Stack · Pioneer 990 · Garantie · für wen (nicht) |
| **6. CTA + Q&A** | 84–90+ | 26–27 | Cart-Link · Dringlichkeit · Fragen · Closing |

**Emotions-Kurve:** Tief (Hamsterrad/Abhängigkeit) → Hoffnung (Wendepunkt) → Möglichkeit (Demo) → Identifikation (Community) → Entscheidung (Pitch). Die zwei emotionalen Höhepunkte: **Slide 4–5 (Abhängigkeit + Wendepunkt)** und **Slide 20 (Community / nicht allein)**.

---

## 4. Slide-für-Slide — Inhalt + Sprechnotizen

> Brand: Creme `#f1ecdd` · Petrol `#12828c` · Dunkelblau `#29556d` · Orange `#dc822e` · Gelb `#f5e555` · Fonts Philosopher + Source Sans 3 · 16:9 · 3–4 Stichpunkte/Slide · frei sprechen.
> Sprechnotizen sind **Erinnerungsstütze, nicht zum Ablesen**. Du-Anrede, fliessend, keine Stakkato-Sätze.

---

### PHASE 1 — STORY / EMOTION (Min 0–20)

#### SLIDE 1 — Titel
```
[Eyebrow, Petrol] Live-Webinar · 24. Juni
[Headline, Philosopher, Dunkelblau]
Schon 1 Jahr im Network —
und immer noch kein richtiges Einkommen?
[Sub, kursiv, Petrol] Ich zeig dir, wie ich als 4-fach-Mama zwei eigene Einkommen aufgebaut hab.
[unten] — Patricia · Mum Life Balance
```
**Sprechnotizen (1 Min):** „Schön, dass du da bist. Ich bin Patricia, vierfache Mama aus dem Appenzellerland. Wenn du schon eine Weile im Network bist und dich fragst, warum trotzdem kein richtiges Einkommen kommt, dann bist du hier goldrichtig. In den nächsten 90 Minuten zeig ich dir, wie ich mir etwas aufgebaut hab, das wirklich meins ist — und das nicht davon abhängt, wie viele Leute gerade mitmachen. Bleib bis zum Schluss dran, denn am Ende öffne ich etwas, auf das einige von euch schon gewartet haben."

#### SLIDE 2 — Wer ich bin (Anti-Bali-Glaubwürdigkeit)
```
[Foto Patricia, rund] + rechts:
4 Kinder · Appenzellerland, 920 m
18 Stunden Business pro Woche
Kein Monat mehr ohne Verkauf — seit Mai 2025
[Zitat, Orange] „Ich bin nicht die Mentorin am Strand. Ich bin die mit vier Kindern und Wäscheberg."
```
**Sprechnotizen (2 Min):** „Damit du weisst, wer hier mit dir spricht: Ich hab keine sechsstellige Postkarte aus Bali, ich hab vier Kinder, einen Garten und genau 18 Stunden die Woche fürs Business — mehr ist mit der Familie nicht drin. Und trotzdem vergeht heute kein Monat mehr, in dem ich nichts verkaufe. Wie ich da hingekommen bin, das erzähl ich dir jetzt ehrlich — auch den Teil, der wehgetan hat."

#### SLIDE 3 — Das Hamsterrad (2023)
```
[Two-Column, Foto müde Mama links]
2023 war ich gefangen.
Vollzeit in der Verwaltung. Vier Kinder. Jeden Sonntag Bauchweh vor Montag.
[Zitat, Orange] „Hoffentlich kann ich morgen arbeiten gehen, falls keins krank wird."
```
**Sprechnotizen (2.5 Min):** „2023 stand ich da, wo vielleicht du gerade stehst. Job, den ich eigentlich mochte, aber jeden Sonntagabend dieses Ziehen im Bauch, weil die ganze Woche wieder ein Logistik-Drahtseilakt war. Mit vier Kindern konnte ich gar nicht normal arbeiten gehen, weil die Betreuung mehr gekostet hätte als ich verdient hab. Ich wollte raus — aber ich wusste nicht wie."

#### SLIDE 4 — Der erste Ausweg, und die bittere Erkenntnis (EMOTIONALER HÖHEPUNKT 1)
```
[Hintergrund Dunkelblau, Text Creme]
Mein erster Ausweg war ein Network.
Und irgendwann hab ich gemerkt:
[Headline gross, Gelb]
„Mein Einkommen hängt davon ab,
wie viele Leute mitmachen — und was die tun."
[unten, Creme] Darauf hatte ich keine Lust mehr.
```
**Sprechnotizen (3 Min):** „Mein erster Ausweg war das Network-Marketing — und versteh mich nicht falsch, ich bin bis heute dabei und liebe vieles daran. Aber irgendwann ist mir etwas aufgefallen, das mir ehrlich keine Ruhe mehr gelassen hat: Mein Einkommen hing nicht nur an mir. Es hing daran, wie viele Frauen ich finde und ob die dann auch wirklich dranbleiben. Ich hab gemerkt, ich hab mir da eine neue Abhängigkeit gebaut, statt frei zu werden. Und genau darauf hatte ich einfach keine Lust mehr. Ich wollte etwas, das wirklich meins ist."

#### SLIDE 5 — Der Wendepunkt (EMOTIONALER HÖHEPUNKT 1, Auflösung)
```
[Hintergrund Petrol, Text Creme/Gelb]
[Eyebrow, Gelb] Der Wendepunkt
Ich hab aufgehört, mein Produkt in den Mittelpunkt zu stellen —
und angefangen, die Transformation zu verkaufen.
[Zitat, Gelb, kursiv] „Eine Mutter darf ein eigenes Leben — und ein eigenes Business haben."
```
**Sprechnotizen (3 Min):** „Der Punkt, an dem sich alles gedreht hat, war diese eine Entscheidung: Ich hab aufgehört, ständig über mein Produkt zu reden, und angefangen, über die Veränderung zu sprechen, die ich Frauen ermögliche. Ab da war ich nicht mehr austauschbar. Ab da kamen Frauen zu MIR — nicht wegen eines Produkts, sondern wegen dem, wofür ich stehe. Und das war der Moment, in dem mein eigenes Einkommen angefangen hat, planbar zu werden."

#### SLIDE 6 — Heute (das „WAS" — Ralf-Prinzip)
```
[Creme, 3 grosse Zahlen-Karten]
Heute:
→ Zwei eigene Einkommensströme — vierstellig
→ Kein Monat ohne Verkauf seit Mai 2025
→ 18 Stunden pro Woche · 4 Kinder · solo, mit KI
[unten, Orange kursiv] Und das Wichtigste: nichts davon hängt mehr davon ab, ob andere mitmachen.
```
**Sprechnotizen (2 Min):** „Heute hab ich zwei Einkommen, die beide meine sind — beide vierstellig im Monat. Kein Monat mehr ohne Verkauf, seit über einem Jahr. Und ich arbeite dafür 18 Stunden die Woche, neben vier Kindern, allein, mit meinen KI-Helfern. Ich zeig dir heute nicht jeden einzelnen Klick — aber ich zeig dir, dass es geht. Und dann zeig ich dir den Weg, wie auch du da hinkommst."

---

### PHASE 2 — REFRAME (Min 20–35)

#### SLIDE 7 — „Du erkennst dich wieder" (3 DM-Fragen)
```
[3 Zitat-Boxen, Orange-Border]
„Wie finde ich überhaupt Kundinnen?"
„Wo soll ich anfangen?"
„Was soll ich denn posten?"
[unten] Diese drei Fragen kriege ich jeden Tag. Von Mamas mit vielen Followern — und null Anfragen.
```
**Sprechnotizen (2 Min):** „Diese drei Fragen bekomme ich jeden einzelnen Tag in die Nachrichten. Und fast immer von Frauen, die schon eine Weile dabei sind, viele Followerinnen haben — und trotzdem fragt keine an. Wenn du dich da gerade wiedererkennst, schreib mir ein kurzes ‚Ja' in den Chat. Du bist damit nämlich überhaupt nicht allein."

#### SLIDE 8 — Die Abhängigkeits-Falle benannt
```
[Weiss, 3 Pain-Karten]
Warum es nicht vorangeht:
01. Du verkaufst ein Produkt — wie hundert andere auch.
02. Dein Umsatz hängt an Provisionen und daran, ob dein Team aktiv ist.
03. Du machst alles selbst — bis spät abends, neben der Familie.
```
**Sprechnotizen (3 Min):** „Lass uns ehrlich sein, warum es bei so vielen nicht läuft. Erstens redest du über ein Produkt, das es genauso bei tausend anderen gibt — da ist nichts, was dich heraushebt. Zweitens hängt dein Einkommen an Provisionen und daran, ob dein Team gerade Lust hat. Und drittens machst du alles allein, oft bis nach dem Ins-Bett-Bringen. Das ist kein Disziplin-Problem. Das ist ein Struktur-Problem — und Struktur kann man ändern."

#### SLIDE 9 — Der Shift (das grosse Reframe)
```
[Petrol-Gradient, Mega-Text]
Der eine Wechsel, der alles verändert:
[gross, Creme]
Weg vom Produkt — hin zur Transformation.
[Gelb] So wirst du unabhängig: von der Konkurrenz, vom Provisions-Glück, vom Zufall.
```
**Sprechnotizen (2.5 Min):** „Der eine Wechsel, der bei mir alles verändert hat, ist dieser: weg vom Produkt, hin zur Transformation. In dem Moment, in dem du nicht mehr ein Fläschchen oder ein Pulver verkaufst, sondern eine echte Veränderung im Leben einer Frau, bist du nicht mehr vergleichbar. Und genau das macht dich unabhängig — von der Konkurrenz, von Provisionen, vom Glück, ob dein Team grad mitzieht."

#### SLIDE 10 — Die 3 Säulen (das Lehr-Gerüst = WAS)
```
[Creme, 3 Säulen-Icons]
Was eine Mama wirklich braucht — 3 Säulen:
01. SICHTBARKEIT — dein eigenes Thema, deine Positionierung
02. EIGENES ANGEBOT — dein zweites Standbein, eigener Umsatz
03. ZEIT + KI — damit es neben der Familie läuft
[unten, Orange] Eine Säule allein reicht nicht. Erst alle drei machen dich frei.
```
**Sprechnotizen (2.5 Min):** „Damit das funktioniert, braucht es drei Dinge — ich nenne sie meine drei Säulen. Erstens Sichtbarkeit mit einem eigenen Thema, damit Frauen wegen DIR kommen. Zweitens ein eigenes Angebot, ein zweites Standbein, das dir echten Umsatz bringt und nicht nur Provision. Und drittens Zeit und KI, damit das Ganze überhaupt neben Kindern und Haushalt läuft. Und jetzt zeig ich dir von jeder Säule, was möglich ist."

---

### PHASE 3 — MÖGLICHKEIT ZEIGEN (Min 35–55)

#### SLIDE 11 — Säule 1 live: Sichtbarkeit & dein Thema (Companion-Demo)
```
[Orange, Weiss-Text]
Säule 1 — live.
Ich zeig dir meinen Instagram-Kundenmaschinen-Companion.
→ Du gibst dein Thema ein → er macht dir Hooks, Captions, Ideen in DEINER Stimme.
[unten kursiv] Schreib mit. Ich zeig dir, DASS es geht.
```
**[LIVE-DEMO — Bildschirm teilen, ~7 Min]:** Companion öffnen → ein Beispiel-Thema eingeben (z.B. „Schlaf für gestresste Mamas") → Companion liefert 3 Hooks + 1 Caption-Gerüst. Punch: „Das hat 40 Sekunden gedauert. Was eine Frau sonst eine Stunde am Abend kostet."
**Sprechnotizen:** „Ich zeig dir jetzt etwas, das ich selbst gebaut hab — meinen Companion für die Instagram-Sichtbarkeit. Ich tippe ein Thema ein, und er gibt mir Hooks und Captions, die nach mir klingen. Schau einfach zu. Ich erklär dir heute nicht jeden Handgriff — ich will dir zeigen, dass das möglich ist, auch ohne dass du technisch bist."

#### SLIDE 12 — Säule 2: Dein eigenes Angebot / Hybrideinkommen
```
[Weiss, Vorher-Nachher]
Säule 2 — dein zweites Standbein:
Vorher: nur Provision, abhängig vom Team.
Nachher: ein eigenes digitales Angebot, das auch verkauft, wenn du schläfst.
[Orange] Das ist dein Hybrid-Einkommen.
```
**Sprechnotizen (2.5 Min):** „Säule zwei ist der Teil, den fast niemand im Network macht — und genau deshalb ist er so wertvoll. Du baust dir ein eigenes kleines Angebot. Etwas Digitales, das dir gehört. Das verkauft sich auch, während du mit den Kindern am Tisch sitzt. Das ist dein zweites Standbein, dein Hybrid-Einkommen — und niemand kann es dir wegnehmen oder die Bedingungen ändern."

#### SLIDE 13 — Säule 3: Zeit + KI (kurze Demo Cockpit/Companion)
```
[Petrol, Creme-Text]
Säule 3 — Zeit zurückholen:
Mein KI-Team übernimmt den Adminkram — Business UND Haushalt.
→ Morgenbriefing · Content-Vorbereitung · Wochenplanung
[Gelb] So reichen 18 Stunden die Woche.
```
**[Kurz-Demo optional, ~3 Min]:** Cockpit-Morgenbriefing kurz zeigen.
**Sprechnotizen (2.5 Min):** „Und die dritte Säule ist der Grund, warum das alles neben der Familie überhaupt geht: Zeit. Ich hab mir KI-Helfer gebaut, die mir den ganzen Kleinkram abnehmen — im Business und sogar im Haushalt. Mein Morgenbriefing kommt, bevor ich überhaupt am Schreibtisch sitze. Genau das macht aus 18 Stunden die Woche ein echtes Einkommen."

#### SLIDE 14 — Recap: das war möglich
```
[Petrol-Gradient]
Was du gerade gesehen hast:
✓ Sichtbarkeit, die Frauen zu DIR bringt
✓ Ein eigenes Angebot, das dir gehört
✓ KI, die dir die Zeit zurückgibt
[Mega-Zitat, Orange] „Wenn ich das kann — mit vier Kindern — dann kannst du das auch."
```
**Sprechnotizen (1.5 Min):** „Das hast du gerade in echt gesehen — keine Theorie. Sichtbarkeit, eigenes Angebot, Zeit durch KI. Und jetzt kommt die ehrliche Frage: Willst du das selbst aufbauen, Schritt für Schritt, mit jemandem an deiner Seite, der genau diesen Weg schon gegangen ist? Dann bleib jetzt dran."

---

### PHASE 4 — PITCH MBA (Min 55–73)

#### SLIDE 15 — Einführung MBA
```
[Creme, zentriert]
Ich hab den ganzen Weg in EIN Zuhause gepackt:
[Headline, Philosopher, Dunkelblau]
Die Mum Business Academy.
[Sub] Alle drei Säulen. Plus die Begleitung, damit du wirklich umsetzt.
```
**Sprechnotizen (2 Min):** „Ich wurde so oft gefragt ‚Patricia, wo lern ich das alles bei dir?', dass ich es in ein einziges Zuhause gepackt hab: die Mum Business Academy. Da drin sind alle drei Säulen — und das, was den Unterschied macht, ob man einen Kurs nur kauft oder ihn wirklich umsetzt."

#### SLIDE 16 — Was drin ist (Bundle-Übersicht)
```
[Weiss, 4 Karten]
Die MBA enthält:
📦 Kurs 1 — Instagram-Kundenmaschine (Sichtbarkeit & Thema)
📦 Kurs 2 — Digitale Produktwelt (dein eigenes Angebot)
📦 Kurs 3 — Mama-CEO (Zeit + KI-System)
👯 Umsetzerinnen-Calls — 2× im Monat (Expertinnen + ich)
💬 Community (Circle) — dein Frauen-Team
```
**Sprechnotizen (2 Min):** „Da sind drei vollständige Kurse drin — einer pro Säule. Dazu zweimal im Monat unsere Umsetzerinnen-Calls, mal mit Expertinnen, mal mit mir direkt. Und eine Community, in der du nicht allein vor dich hin werkelst, sondern echte Frauen an deiner Seite hast, die genau das Gleiche aufbauen."

#### SLIDE 17 — Kurs 1: Instagram-Kundenmaschine (Transformation)
```
[Creme, „01"]
Kurs 1 — Instagram-Kundenmaschine
Nachher: Du hast dein Thema, deine Positionierung — und Frauen fragen von selbst an.
```
**Sprechnotizen (1.5 Min):** „Im ersten Kurs findest du dein Thema und deine Positionierung. Am Ende hast du keine leeren Followerinnen mehr, sondern Frauen, die von selbst in deine Nachrichten kommen und fragen, wie sie mit dir arbeiten können."

#### SLIDE 18 — Kurs 2: Digitale Produktwelt (Transformation)
```
[Weiss, „02"]
Kurs 2 — Digitale Produktwelt
Nachher: Du hast dein eigenes Angebot, das dir gehört und auch ohne dich verkauft.
```
**Sprechnotizen (1.5 Min):** „Im zweiten Kurs baust du dein eigenes digitales Angebot. Das ist dein zweites Standbein — der Teil, der dich unabhängig macht. Etwas, das dir gehört und das verkauft, auch wenn du grad mit den Kindern unterwegs bist."

#### SLIDE 19 — Kurs 3: Mama-CEO (Transformation)
```
[Creme, „03"]
Kurs 3 — Mama-CEO: Zeit + KI
Nachher: Dein KI-Team nimmt dir den Adminkram ab — du arbeitest mit Menschen statt mit To-Do-Listen.
```
**Sprechnotizen (1.5 Min):** „Und im dritten Kurs holst du dir die Zeit zurück. Du baust dir deine eigenen KI-Helfer, genau wie ich. Am Ende verbringst du deine Stunden mit Menschen und mit dem, was zählt — nicht damit, abends noch Posts zu tippen."

#### SLIDE 20 — Die Community (EMOTIONALER HÖHEPUNKT 2 — der Differenzierer)
```
[Petrol, Creme-Text]
Das, was den Unterschied macht:
Du bist nicht allein.
[gross, Gelb] Zweimal im Monat treffen wir uns. Echte Frauen, echte Fragen, echte Schritte.
[unten] Genau hier setzen die meisten Kurse um — oder eben nicht.
```
**Sprechnotizen (2.5 Min):** „Und jetzt der Teil, der mir am meisten am Herzen liegt. Du hast bestimmt schon Kurse gekauft, die dann im digitalen Regal verstaubt sind. Ich auch. Der Grund ist fast immer derselbe: Man ist allein. In der Academy bist du das nicht. Zweimal im Monat sehen wir uns, du stellst deine Fragen, du gehst mit einem konkreten nächsten Schritt raus. Das ist der Moment, in dem aus ‚gekauft' endlich ‚gemacht' wird."

#### SLIDE 21 — Boni (in der MBA enthalten)
```
[Petrol-Gradient, 3 Bonus-Karten]
🎁 Bonus 1 — Mein Instagram-Companion (der aus der Demo)
🎁 Bonus 2 — PIA: deine KI-Mentorin, die mit deinen Daten arbeitet (Stufe 0 ready)
🎁 Bonus 3 — Mein Notion-Business-Brain als Vorlage zum Duplizieren
[unten] Alles dabei — kein Aufpreis.
```
**Sprechnotizen (1.5 Min):** „Und obendrauf gibt's drei Boni, die ich sonst nicht aus der Hand gebe. Genau den Companion, den du gerade in der Demo gesehen hast. Dann PIA — eine KI-Mentorin, die mit deinen eigenen Daten arbeitet und dir Hooks und Texte in deiner Stimme baut. Und meine echte Notion-Vorlage, die du einfach duplizierst, statt bei Null anzufangen."

---

### PHASE 5 — PREIS + RISK + ANTIKUNDEN (Min 73–84)

#### SLIDE 22 — Wert-Stack
```
[Weiss, aufaddierende Liste]
Was du bekommst — einzeln:
Kurs 1 ........... 333
Kurs 2 ........... 333
Kurs 3 ........... 333
Umsetzerinnen-Calls + Community ... unbezahlbar (laufende Begleitung)
Boni ............. 3× Patricia-eigen
─────────────────
Einzeln allein: über 990 — nur für die Kurse.
```
**Sprechnotizen (2 Min):** „Lass uns über den Wert reden. Allein die drei Kurse kosten einzeln zusammen schon über 990 Franken — und da sind die Calls und die Community noch gar nicht drin, die ja monatlich weiterlaufen. Du bekommst also den kompletten Weg plus die Begleitung, die dafür sorgt, dass du ihn auch wirklich gehst."

#### SLIDE 23 — Pricing (Pioneer-Drama)
```
[Weiss, Preis-Karten]
[Eyebrow, Petrol] Erste Runde · Pioneer-Plätze begrenzt
[FEATURED] PIONEER
CHF 990
Gründungsmitglied · erste Runde · niedrigster Preis, den es je gibt
[daneben] Späterer Listenpreis: CHF 1347
[unten] Zahlplan möglich: z.B. 3× CHF 363. Cart schliesst: [Datum].
```
**Sprechnotizen (3 Min):** „Jetzt zum Preis. Der spätere Listenpreis der Academy ist 1347. Heute, als Pioneer der allerersten Runde, kommst du für 990 rein — das ist der niedrigste Preis, den es je geben wird, weil du mir hilfst, die erste Runde zu gestalten, und dafür am nähesten an mir dran bist. Und damit das machbar ist neben der Familie, kannst du auch in Raten zahlen, zum Beispiel dreimal 363. Die Plätze in der Pioneer-Runde sind begrenzt, und der Cart schliesst am [Datum]."

#### SLIDE 24 — Garantie / Risk-Reversal (wichtig bei kalt)
```
[Creme, Siegel-Optik]
Mein Versprechen an dich:
Komm rein, schau dir den ersten Kurs und den ersten Call an.
Wenn du das Gefühl hast, das ist nichts für dich — sag mir innerhalb von 14 Tagen Bescheid.
[Orange] Dann bekommst du dein Geld zurück. Ohne Diskussion.
```
**Sprechnotizen (2 Min):** „Weil du mich vielleicht heute zum ersten Mal siehst, nehm ich dir das Risiko ab. Du kommst rein, schaust dir den ersten Kurs und den ersten Call an. Wenn du dann merkst, das passt für dich nicht, schreibst du mir innerhalb von zwei Wochen — und ich geb dir dein Geld zurück, ohne Diskussion. Das Einzige, was ich von dir brauche, ist, dass du wirklich reinschaust."

#### SLIDE 25 — Antikunden (Ja / Nein)
```
[Creme, 2 Spalten]
✓ JA, wenn du... seit einer Weile dabei bist · bereit bist umzusetzen · dein eigenes Standbein willst · KI nutzen willst (auch wenn du heute nicht weisst wie)
✗ NEIN, wenn du... ein schnelles Wunder ohne Arbeit suchst · in 6 Monaten reich werden willst · Kurse sammelst und nie anfängst · willst, dass ich's für dich mache
```
**Sprechnotizen (2 Min):** „Eine kurze Ehrlichkeit, bevor du klickst. Die Academy ist für dich, wenn du schon eine Weile dranbleibst und jetzt wirklich umsetzen willst — dein eigenes Standbein, mit KI an deiner Seite. Sie ist nicht für dich, wenn du ein schnelles Wunder ohne Arbeit suchst oder seit Jahren Kurse kaufst und nie anfängst. Ich verspreche dir keinen Reichtum in sechs Monaten. Ich zeig dir einen ehrlichen Weg — den du gehst."

---

### PHASE 6 — CTA + Q&A (Min 84–90+)

#### SLIDE 26 — Call-to-Action
```
[Orange, Weiss-Text, Mega-CTA]
Sichere dir deinen Pioneer-Platz.
CHF 990 · erste Runde · Plätze begrenzt
🔗 [mumlifebalance.ch/mba]   [QR-Code]
[unten] Link ist im Chat. Schnell-Bonus nur heute.
```
**Sprechnotizen (1.5 Min):** „Der Cart ist jetzt offen. Den Link findest du im Chat und hier auf dem Bildschirm. Wenn du heute dabei bist und dir deinen Pioneer-Platz sicherst, bekommst du den Schnell-Bonus dazu. Klick rein, trag dich ein — und dann sehen wir uns drinnen. Und jetzt nehm ich mir Zeit für deine Fragen."

#### SLIDE 27 — Q&A + Closing
```
[Dunkelblau, Patricia-Foto rund]
Deine Fragen.
[Mega-Zitat, Orange, kursiv] „Funktionieren war gestern."
— Patricia
```
**5 Backup-Q&A (falls Chat zögert):**
1. *„Reichen mir die paar Stunden, die ich hab?"* → Ja, weil du in Säule 3 zuerst Zeit schaffst, bevor du mehr tust — die Lektionen sind kurz, keine ist länger als eine Kaffeepause.
2. *„Ich bin nicht technisch — geht das trotzdem?"* → Genau dafür ist es gemacht. Bis vor Kurzem wusste ich auch nicht, was ein KI-Helfer ist. Du wirst an die Hand genommen, Schritt für Schritt.
3. *„Ich bin im Network — passt das, oder ist das ein Widerspruch?"* → Es passt perfekt. Du baust dir ein zweites, eigenes Standbein neben deinem Network — damit du nicht mehr nur von Provisionen und deinem Team abhängst.
4. *„Was, wenn ich nicht zu jedem Call kann?"* → Alle Calls werden aufgezeichnet, und du kannst deine Fragen vorher einreichen. Aber live ist live — da passiert am meisten.
5. *„Wann geht's los und wie lange hab ich Zugriff?"* → Start ist [Datum]. Die Kurse und Aufzeichnungen bleiben deine, die Community läuft weiter.

**Closing (letzte 1 Min):** „Danke, dass du die ganze Zeit dabei warst. Wer sich heute den Pioneer-Platz sichert, ist von Anfang an am nähesten dran an mir und an den anderen Frauen. Der Link ist im Chat, der Schnell-Bonus gilt nur heute. Ich freu mich, dich drinnen zu sehen. Und denk dran — funktionieren war gestern."

---

## 5. Das Angebot — MBA Offer-Stack (Referenz)

| Bestandteil | Einzel-Wert | In MBA |
|---|---|---|
| Kurs 1 — Instagram-Kundenmaschine | CHF 333 | ✓ |
| Kurs 2 — Digitale Produktwelt | CHF 333 | ✓ |
| Kurs 3 — Mama-CEO (Zeit + KI) | CHF 333 | ✓ |
| Umsetzerinnen-Calls (2×/Monat, Expertinnen + Patricia) | laufende Begleitung | ✓ |
| Community (Circle) | laufend | ✓ |
| Bonus 1 — Instagram-Companion | Patricia-eigen | ✓ |
| Bonus 2 — Notion-Business-Brain-Vorlage | Patricia-eigen | ✓ |
| Bonus 3 — Schnell-Entschlossenen-Bonus | nur Live/Heute | ✓ |
| **Pioneer-Preis (erste Runde)** | | **CHF 990** |
| **Späterer Listenpreis** | | **CHF 1347** |

→ Offene Entscheidung für dich: **Was genau ist Bonus 3** (Schnell-Bonus)? Vorschläge: eine 1:1-Audio-Woche mit dir · ein zusätzlicher Live-Onboarding-Call nur für Pioneers · ein 4. Mini-Companion. Sag mir, was sich für dich gut anfühlt und realistisch ist.

---

## 6. Preis-Dramaturgie & Cold-Traffic-Hebel

- **Pioneer 990 vs. Liste 1347** = ehrliche Verknappung über die „erste Runde" (Gründungsmitglieder gestalten mit, sind am nähesten dran). Kein Fake-Countdown.
- **Zahlplan** (z.B. 3× 363 oder 6× 185) — bei kaltem Publikum fast Pflicht, senkt die mentale Schwelle ohne den Preis zu drücken.
- **Fast-Action-Bonus** (Bonus 3) nur für Live-Teilnehmerinnen / Kauf innerhalb von X Stunden → belohnt Entscheidung, ethisch (Bonus statt Rabatt).
- **Risk-Reversal / 14-Tage-Garantie** → nimmt der fremden Frau das Risiko, macht den 990-Sprung machbar.
- **Cart-Fenster:** klar kommunizieren (Webinar-Tag → Cart offen bis [Datum], z.B. 4–7 Tage). Danach Pioneer-Preis weg.

> ⚠️ **Cold-Traffic-Ehrlichkeit:** Ein 990-Verkauf direkt aus kaltem Ads-Webinar ist anspruchsvoll (Conversion realistisch tief, 1–3 %). Zwei Optionen, die das abfedern — du entscheidest:
> 1. **Retargeting-Wärme:** Ads erst auf einen kleinen 0€-Leadmagneten (du hast 6 Stück!), dann Webinar-Einladung an die schon leicht erwärmte Liste. Höhere Show-up + Conversion.
> 2. **Call-Alternative beim High-Ticket:** statt direktem Cart-Klick ein „Buch dir ein kurzes Gespräch"-Schritt für Unentschlossene → du oder eine Expertin schliesst im Gespräch. Für 990 oft stärker als reiner Selbst-Checkout.

---

## 7. Cold-Funnel: Anmeldung → Show-up → Pitch → Replay → Evergreen

1. **Meta-Ad** (Transformation/Freiheit-Hook, KEIN Income-Claim) → Anmelde-Landingpage (WP, vgl. [[reference_ki-webinar-lp-technik]] · Page-Struktur wie KI-Webinar-LP 3675).
2. **Anmelde-LP** → AC-Form → Tag `mba-webinar-anmeldung` → Bestätigungs-Mail mit Termin + Kalender-Eintrag.
3. **Reminder-Sequenz** (Show-up-Booster): 1 Woche / 24 h / 1 h vorher + „bring Notizblock mit". Show-up bei kalt ist der grösste Hebel.
4. **Live-Webinar** → Cart-Open am Ende → Tag `mba-pitch-start`.
5. **Replay-Fenster** (48–72 h): Replay-Mail + Cart-Reminder + Fast-Bonus-Auslauf.
6. **Nicht-Käuferinnen** → laufen in die **MBA-Pitch-Automation** (Webinar-Replay als Evergreen-Brücke, siehe [[umbau-plan]] Phase 1 + 4). Tag `mba-kauf` beendet die Automation.

🚨 **Vor Ad-Aktivierung:** `curl -I` auf den Anmelde-Link — die 44-CHF-Lektion aus Mai darf sich nicht wiederholen ([[feedback_KRITISCH-ad-link-pre-launch-test]]).

---

## 8. Promo-Plan (grobe Zeitachse — Daten einsetzen)

| Phase | Wann | Was |
|---|---|---|
| Anwärmen | −10 bis −3 Tage | Ads starten · organische Story/Reels zum Abhängigkeits-Thema · Anmeldungen sammeln |
| Last Call | −2 bis −1 Tag | „Morgen live"-Reminder · Behind-the-Scenes · letzte Anmeldungen |
| Webinar-Tag | Tag 0 | Live 09:00 (dein Power-Window) · Cart-Open · Story-Cascade vor/während/nach |
| Replay-Fenster | Tag +1 bis +3 | Replay-Mail · Cart-Reminder · Fast-Bonus läuft aus |
| Cart-Close | Tag +X | Letzte-Stunden-Cascade · Cart zu · Pioneer-Preis endet |
| Evergreen-Übergang | danach | Aufzeichnung schneiden → in MBA-Pitch-Automation einhängen |

→ Content baust du über `/freitag-hooks` + `/reels` + `/karussell` (Pillar: Abhängigkeit→Freiheit, eigenes Standbein, KI-Möglichkeit). Ads über `scripts/meta-ads/` (vgl. `create-webinar-ad.js` / `clone-webinar-ad.mjs`).

---

## 9. Umsatz-Realitäts-Check (Bezug 40k-Jahresziel)

```
Pioneer-Preis: CHF 990
40'000 ÷ 990 ≈ 40 MBA-Verkäufe/Jahr für das gesamte Jahresziel allein über MBA.

Webinar-Szenario (eine Runde, kalt + warm gemischt):
• 80 Anmeldungen → ~30 % Show-up = 24 live
• Cold-Conversion 8–12 % auf Live + Replay → 2–4 Pioneer-Verkäufe
• 3 Verkäufe × 990 = CHF 2'970 aus EINER Webinar-Runde

→ Realistischer Hebel: Webinar als wiederholbares + Evergreen-Asset.
   3–4 Verkäufe pro Runde × mehrere Runden + laufender Evergreen-Strom
   = der Pfad, auf dem die MBA das 40k-Ziel trägt, statt aus einem Single-Event.
```
**Ehrliche Einordnung:** Ein einzelnes kaltes Webinar trägt nicht das ganze Jahr. Der Wert liegt darin, dass du es **einmal sauber baust, live hältst, aufnimmst — und dann dauerhaft im Funnel arbeiten lässt.** Genau dafür ist „Live zuerst → Evergreen" goldrichtig.

---

## 10. Tech & Logistik (Webinar-Tag)

- Zoom-Webinar als „Practice Session" 30 Min vorher · Kamera/Mikro/Screenshare testen
- Companion-Demo vorbereiten (Tab offen, Beispiel-Thema bereit, Fallback-Screenshot falls Demo hakt)
- Cart-Link kopierbereit + im Chat ab Pitch-Beginn · QR-Code auf Slide 26
- Zweites Gerät als Backup · Wasser · Notiz mit 5 Backup-Q&A
- Aufnahme AN (für Evergreen!) · nach dem Webinar: Replay-Link + Schnitt-To-do

---

## 11. Nächste Schritte (To-Dos für dich)

- [ ] **Webinar-Datum + Cart-Close-Datum festlegen** (ich trage sie dann überall ein)
- [ ] **Titel A / B / C picken** (oder eigenen)
- [ ] **Bonus 3 definieren** (Schnell-Entschlossenen-Bonus)
- [ ] **Zahlplan bestätigen** (3× 363? 6× 185?)
- [ ] **Cold vs. Retarget-Strategie entscheiden** (Sektion 6 — direkt kalt oder über Leadmagnet anwärmen?)
- [ ] Danach baue ich dir auf Wunsch: die **Anmelde-Landingpage** (`/wp`), die **Reminder-Mail-Sequenz**, die **Meta-Ad** (Income-Claim-safe) und die **Canva-Slides** aus diesem Skript.

---

## Compliance-Check
- ✅ Keine erfundenen Zahlen — „2x vierstellig" = Patricias echte Realität (vgl. Vollprofil); KEINE externen Fake-Stats verwendet
- ✅ Kein „6-stellig", kein „Hustle/Grind", kein „Stell dir vor", kein „Kennst du das?"
- ✅ Keine Mentor-Namen (Julia/Ralf etc.) in Slides oder Sprechnotizen — nur intern in Sektion 1
- ✅ Keine doTERRA-Heilversprechen · Network nur als ehrlicher Teil der Story, kein Produkt-Pitch
- ✅ Anti-Bali-Frame · Patricia im Fokus · Schweizer ss · Du-Anrede · keine Stakkato-Sätze
- ⚠️ **Meta-Income-Claim:** „2x vierstellig" gehört ins Webinar, NICHT in die Ad — sonst Ad-Ablehnung
- ⚠️ **Ad-Link vor Aktivierung testen** (curl -I)
- ⚠️ Wendepunkt-/Network-Abhängigkeits-Story: prüf für dich, ob du sie live so offen erzählen willst (du darfst — kein Zwang)

---

## 🔗 Verwandte Notizen
- [[umbau-plan]] — MBA-Pitch-Automation (Evergreen-Ziel des Webinars)
- [[reference_ki-webinar-lp-technik]] — Anmelde-LP-Technik (WP/AC/Pixel)
- [[feedback_KRITISCH-ad-link-pre-launch-test]] — Ad-Link-Pflichttest
- [[feedback_KRITISCH-mba-bundle-struktur]] — MBA = Bundle aus 3 Kursen + Calls
- [[06-webinar-pitch-slides]] — Mama-CEO-Webinar (Struktur-Vorlage)
