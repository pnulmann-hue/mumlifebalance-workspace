---
tags: [produkt, doterra]
---

# 📦 Das Produktpaket der 21-Tage-Energie-Routine

**Was die Kundin bestellt (= dein Provisions-Punkt), verpackt in deine gratis 21-Tage-Begleitung.**
Grundlage: Patricias echte tägliche Routine aus der Wendepunkt-Story. Produktnamen mit echten doTERRA-Bezeichnungen (recherchiert, nicht erfunden).

> ⚠️ **Du bestätigst:** genaue CHF-Preise, PV (Point Value) und die konkrete Kit-/Enrollment-Variante aus deinem doTERRA-Backoffice. Preise erfinde ich nicht.

---

## ⭐ Das Einschreibe-Paket („Energie-Paket") — muss ≥150 PV ergeben

**Kern (Patricias echte 3 Täglichen) — ~125 PV:**

| Produkt | Was es abdeckt (deine Bausteine) |
|---|---|
| **VMG+** (Microplex VMz) | Mikronährstoffe |
| **PB Assist+** | Darm — dein Darm-Hormon-Modell (Präbiotika + Probiotika) |
| **Omega 3** | Entzündung / Energie / klarer Kopf |

→ Protein läuft über die **Ernährung** (30g/Mahlzeit, 1/3-Teller-Regel) — kein Produkt nötig.

**PV-Auffüller auf 150 (Einschreibegrenze) — EMPFOHLEN: ClaryCalm-Roller**

| Zusatz | Warum genau das |
|---|---|
| ⭐ **ClaryCalm-Roller** (doTERRAs Frauen-Mischung, fertig verdünnt) | **On-Brand:** doTERRAs Frauen-Blend → trifft die Positionierung „Hormone natürlich begleiten" direkt. Ist ein **Signature-Produkt** statt austauschbar. Enthält u.a. Lavendel/Kamille/Ylang Ylang → dient **zugleich** als Abend-/Runterkomm-Roller (Tag 3). Fertig verdünnt = kein Kokosöl nötig. |

**🚨 Compliance-Regel für ClaryCalm:** NIE „reguliert/balanciert die Hormone" oder „hilft bei Hormonbeschwerden" (Heilversprechen). NUR „bei mir"-Lifestyle-Frame: *„mein Wohlfühl-Roller"* / *„der, den ich abends nehm / an den Tagen, wo alles zu viel ist"*.

**Alternative:** **Serenity-Roller** (Ruhe/Schlaf) — compliance-unkomplizierter, aber weniger on-brand. · **TerraZyme** (Verdauung→Energie) — kein Ritual-Erlebnis.
**Verworfen:** Öltrio Lavendel/Peppermint/Lemon → zu teuer + bräuchte Kokosöl (Trägeröl) → 4 Produkte.

> ⚠️ **Patricia im Backoffice bestätigen:** Kern (125) + ClaryCalm-Roller ≥ **150 PV**. Falls knapp → Diffuser oder ein zweiter Roller dazu.

---

## ➕ Optionale Add-ons (Upsell, kein Muss)

| Add-on | Wofür |
|---|---|
| **doTERRA Protein** (Plant Protein Schoko/Vanille) | zum „Pimpen", wenn's mal schnell gehen muss |
| **Diffuser** | für die Öle zu Hause — macht das Abend-Ritual schöner |

---

## 💶 Enrollment-Logik + der Verkaufs-Link (Tag 3)

**Verkaufsweg (Patricia-Entscheidung): direkter Paket-Link in Tag 3.**

Damit du dabei **keine Provision verlierst**: KEIN normaler Retail-Produktlink, sondern ein
**vorausgefüllter Warenkorb / Einschreibe-Link (doTERRA „Shareable Cart")** mit dem ganzen
Paket (Kern + Öltrio) drin. Dann kauft sie über EINEN Klick **und** wird als **Vorteilskundin
mit ≥150 PV eingeschrieben** — Direktkauf UND sauberes Setup.

**✅ Cart-Link steht:** https://doterra.me/b0yKEX (in Tag-3-Mail eingesetzt)

1. Cart-Inhalt: VMG+ · PB Assist+ · Omega 3 · ClaryCalm-Roller (≥150 PV).
2. Als Einschreibe-/Vorteilskunden-Cart konfigurieren (Wholesale-Preis).
3. Optional Loyalty-Order (monatliche Bestellung) gleich mitanbieten → **wiederkehrende Provision**.
4. AC-Tags: `kunde` (56) / `vorteilskunde` (57) / `wellness_advocate` (58) — vorhanden.

**Offen für dich:**
- [ ] Link einmal testen (öffnet Cart korrekt?) + prüfen: Paket ≥150 PV
- [ ] CHF-Preise (Wholesale) — optional für die Paket-Übersicht in der Mail

---

## 🎁 Was drumherum kommt (gratis, macht's zum „Paket statt nur Produkt")

Das ist der Unterschied zu „nur Produkte verkaufen": Die Kundin kauft die Produkte, aber sie **erlebt ein begleitetes Programm**:
- 21-Tage-Begleitung (siehe [03-automatisierung-ki-konzept.md](03-automatisierung-ki-konzept.md) → Bot)
- Padlet „Wohlfühlen leicht gemacht" (Anwendungen)
- Öl-Bibliothek-Bot für Fragen
- geführte Telegram-Gruppe

→ **Verkauft wird die Transformation** („raus aus dem Funktionsmodus"), die Produkte sind das Werkzeug.

---

## 🔗 Verwandte
- [[01-produkttreppe]] · [[03-automatisierung-ki-konzept]] · [[07-21-tage-begleitung]]

## Quellen (Produkt-Recherche)
- [doTERRA Lifelong Vitality Pack — offiziell EU/DE](https://shop.doterra.com/de/de_de/create-account/lifelong-vitality-pack/) · [LLV PIP (PDF)](https://media.doterra.com/eu/de/pips/lifelong-vitality-pack.pdf)
- [PB Assist+ — offiziell DE](https://www.doterra.com/DE/de_DE/p/digestzen-pb-assist) · [doTERRA Protein Schoko](https://shop.doterra.com/DE/de_DE/shop/doterra-chocolate-plant-protein/)
