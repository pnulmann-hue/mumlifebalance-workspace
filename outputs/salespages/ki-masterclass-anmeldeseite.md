---
tags: [produkt, salespage, ki-launch, masterclass]
---

# Masterclass „Vom Chat zur Mitarbeiterin“ — Anmeldeseite

**Stand:** 25.09.2026 · **Status:** Entwurf, wartet auf Patricias Freigabe
**Ziel-Seite:** `mumlifebalance.ch/ki-masterclass/` (WP-Entwurf, noch nicht angelegt)
**Termin:** Donnerstag, 8. Oktober 2026, 09:00, Zoom, rund 45 Minuten (verschoben am 25.09., vorher Di 13.10. 20:00)
**Formular:** ActiveCampaign, Kopie von Formular 55 (siehe unten „Technik“)
**Nach der Anmeldung:** Danke-Seite mit dem Stimm-Check als Geschenk

---

## Die Leitplanke für diese Seite (und die Masterclass)

Patricia, 25.09.: *„im webinar erkläre ich nicht wie sondern WAS möglich ist. ich zeige
meine Assistenten, ich zeige vor allem mein Cockpit, ich zeige wie ich skill gemacht habe
und ich zeige auch, was ich mit einem telegramtool dem kochbot erreicht habe.“* —
und: *„es geht ja darum, dass ich zeige WAS ist wichtig. KI personalisieren und genau
sagen, was zu tun ist ohne immer erneut zu überlegen.“*

Das deckt sich mit der Webinar-Methode, die Patricia im Elevate-Call gelernt hat:
**ein Verkaufs-Webinar ist kein Workshop.** Es zeigt, was möglich ist, und macht Lust.
Das Wie gehört in den Kurs. Die Seite verspricht deshalb **Zuschauen und Staunen**,
keinen Kurs am Abend. Die zwei Kernbotschaften:

1. **Personalisieren** — die KI weiss, wer du bist, für wen du schreibst und wie du klingst.
2. **Genau sagen, was zu tun ist** — einmal festgehalten, und ab dann fängst du nicht
   jede Woche wieder bei null an.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 1: Hero + Anmeldung
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Eyebrow:** Kostenlose Live-Masterclass · Donnerstag, 8. Oktober, 9 Uhr

**Headline:** Wie oft hast du deiner KI dieses Jahr schon erklärt, wer du bist?

**Unterzeile:** Am Donnerstagmorgen zeig ich dir live, wie meine KI mitarbeitet, ohne dass
ich jedes Mal von vorne erklären muss: mein Cockpit, meine Assistenten und den Bot, der
mir die Menüplanung für die ganze Woche abnimmt. Du schaust einfach zu, und danach weisst
du, was bei dir der erste Schritt ist.

**Formular:** Vorname · E-Mail

🔘 **Ich bin am Donnerstag dabei**

*Unter dem Knopf, klein:* Die Aufzeichnung bekommen alle, die sich anmelden, auch wenn du
am Morgen nicht kannst.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 2: Das kennst du
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Alle sagen, KI spart Zeit, und bei dir kostet sie welche

Du hast ChatGPT oder Claude schon ausprobiert, vielleicht sogar ziemlich oft. Du tippst
dein Thema hinein, für wen du schreibst, was du anbietest, und bekommst einen Text zurück, der
nach irgendwem klingt, nur nicht nach dir. Also schreibst du ihn um, und am Ende hättest
du ihn fast schneller selbst geschrieben.

Und am nächsten Montag fängst du wieder bei null an, weil sich die KI an nichts erinnert
und du jedes Mal neu überlegen musst, wie du ihr das eigentlich sagst.

Das liegt weder an dir noch an der KI. Es fehlen zwei Dinge, und genau die
zeig ich dir am Donnerstag.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 3: Die zwei Dinge
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Was eine KI braucht, damit sie wirklich mitarbeitet

**1 · Sie muss dich kennen.**
Wer du bist, für wen du schreibst, was du anbietest und wie du klingst. Einmal sauber
festgehalten, und jeder Text danach fängt dort an, statt bei null.

**2 · Sie muss wissen, was zu tun ist.**
Die Arbeit, die du jede Woche wieder machst, steht einmal als Auftrag da, Schritt für
Schritt. Dann sagst du „mach die Caption für Mittwoch“, und sie weiss, was das bei dir
heisst.

Mit diesen zwei Dingen wird aus einem Chat, in dem du jedes Mal alles erklärst, eine
Mitarbeiterin, die weiss, wie du arbeitest.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 4: Was du am Donnerstag siehst
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Kein Vortrag mit Folien, du schaust mir über die Schulter

- **Mein Cockpit** — die eine Seite, auf der meine ganze Content-Woche vorbereitet
  liegt. Ich prüfe, gebe frei, und der Rest läuft.
- **Meine Assistenten** — was sie für mich erledigen, jeden Tag und ohne dass ich
  daran denken muss.
- **Wie ein Skill entsteht** — ich zeige dir an einem echten Beispiel, wie aus einer
  Arbeit, die ich immer wieder gemacht habe, ein fester Auftrag wurde.
- **Den Kochbot** — für eine saubere Menüplanung habe ich früher gut ein, zwei Stunden
  gebraucht, mit Überlegen und Einkaufsliste schreiben. Heute beantworte ich drei Fragen:
  Ist nächste Woche etwas Besonderes, weil ich wenig Zeit habe oder Besuch kommt? Muss
  etwas weg? Daraus stellt er mir im Hintergrund aus meinen eigenen Rezepten die
  Menüliste samt Einkaufsliste zusammen, und alles kommt fix fertig aufs Handy. Das dauert
  fünf Minuten, zehn, wenn mir mal etwas nicht passt. Kochen muss ich allerdings immer
  noch selbst.

Du musst nichts installieren und nichts mitschreiben. Es geht darum, dass du siehst, was
heute schon möglich ist, auch für eine Mama, die keine Technikerin ist.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 5: Für wen
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Die Masterclass ist für dich, wenn du …

- KI schon benutzt und trotzdem jeden Text umschreibst
- ein eigenes Business hast oder im Network arbeitest und deine Zeit knapp ist
- genug Tools ausprobiert hast und dir eines wünschst, das mitdenkt
- neugierig bist, was andere Mamas mit KI schon machen

**Eher nicht für dich**, wenn du eine Programmier-Schulung suchst. Am Donnerstag wird
nichts programmiert.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 6: Wer ich bin
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Ich bin Patricia, Mama von vier Kindern, und ich habe 18 Stunden die Woche

Ich baue mein Business im Appenzellerland neben einem vollen Familienalltag auf, und
für alles Geschäftliche bleiben mir rund 18 Stunden in der Woche. In diesen 18 Stunden
steckt:

- **jeden Tag Story und Feed**, auf zwei Instagram-Kanälen
- **jeden Monat ein kostenloses und ein bezahltes Produkt**
- **mein Network** mit der Betreuung meiner Kundinnen und meines Teams
- **die ganze Administration**

Das wäre ohne meine KI-Assistenten nie möglich. Heute laufen bei mir 13 davon, und jeder
nimmt mir eine Arbeit ab, die ich früher jede Woche von Hand gemacht habe.

Ich zeige dir am Donnerstag nichts, was ich nicht selbst jeden Tag benutze.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 7: Termin + zweites Formular
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Donnerstag, 8. Oktober, um 9 Uhr morgens

Rund 45 Minuten über Zoom. Kamera an oder aus, wie es dir passt. Am Ende erzähl ich dir,
wie du dir das selbst aufbauen kannst, wenn du willst, und du kannst mir deine Fragen
stellen.

Die Aufzeichnung kommt noch am selben Tag per Mail, aber nur an die, die angemeldet sind.

**Formular:** Vorname · E-Mail

🔘 **Ich bin am Donnerstag dabei**

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 8: Fragen
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Kostet das etwas?**
Nein. Die Masterclass ist kostenlos. Am Ende stelle ich meinen Kurs vor, für die, die
es selbst aufbauen wollen. Wer nur zuschauen will, schaut nur zu.

**Ich kann am Donnerstag nicht, lohnt sich die Anmeldung trotzdem?**
Ja. Die Aufzeichnung geht am Donnerstagnachmittag an alle, die angemeldet sind.

**Ich hab von KI keine Ahnung. Verstehe ich das überhaupt?**
Genau dafür zeige ich es, statt es zu erklären. Du musst nichts können, du musst nur
zuschauen.

**Welche KI benutzt du?**
Hauptsächlich Claude. Was ich zeige, gilt aber auch, wenn du heute mit ChatGPT arbeitest.

**Bin ich in der MBA dabei, brauche ich das?**
Komm gern. Den Kurs, den ich am Ende vorstelle, hast du in der MBA schon gratis dabei.

---

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 BLOCK 9: Letzter Aufruf
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**Überschrift:** Einmal erklärt, und ab dann weiss sie es

Du hast deiner KI schon oft genug gesagt, wer du bist. Am Donnerstag siehst du, wie es
aussieht, wenn du es ihr nur einmal sagen musst.

🔘 **Ich bin am Donnerstag dabei**

---

## Danke-Seite (nach der Anmeldung)

**Überschrift:** Fast geschafft, schau kurz in dein Postfach

Ich habe dir gerade eine Mail geschickt. Klick dort auf „Ja, ich bin dabei“, erst dann kommt der
Zoom-Link für Donnerstag, 8. Oktober, 9 Uhr. Am Mittwochabend erinnere ich dich nochmal.
Keine Mail da? Dann schau im Spam oder im Werbung-Ordner nach.

**Und damit du nicht bis Donnerstag warten musst:** Mach den Stimm-Check. In rund 15
Minuten hast du die sechs Angaben, die deine KI von dir braucht, damit sie klingt wie du,
und einen ersten Text, den du so posten würdest. Das ist genau das „Sie muss dich
kennen“ von oben, schon zum Mitnehmen.

🔘 **Zum Stimm-Check**

*(Umgekehrt steht auf der Danke-Seite des Stimm-Checks der Knopf zur Masterclass —
so führen die beiden Seiten ineinander, egal wo sie einsteigt.)*

---

## Technik (für Patricia)

- **Formular:** In ActiveCampaign Formular **55** duplizieren → Name „Masterclass KI
  Okt 2026“ · Felder Vorname + E-Mail · Aktion: Liste Hauptliste + Tag
  **`0€ KI Webinar`** (100) · Weiterleitung auf `/ki-masterclass-danke/`.
  Die ID des neuen Formulars mir schicken, dann baue ich es als iframe ein (wie auf
  `/ki-webinar/`, Seite 3675).
- **Automation:** Trigger Tag `0€ KI Webinar` → Bestätigung sofort ·
  Mi 7.10. 19:00 Erinnerung · Do 8.10. 08:55 „Wir sind live“ ·
  Do 8.10. 16:00 Aufzeichnung · dann die Pitch-Mails (Plan im Launch-Kalender).
- **Zoom-Link:** fehlt noch — kommt in die Bestätigungs- und Erinnerungs-Mails.

## Offen vor der Freigabe

*(alles geklärt am 25.09.: Kochbot-Zahlen, 18 Stunden und 13 Bots von Patricia bestätigt)*

---

## 🔗 Verwandte Notizen

- [[../produkte/ki-treppe/07-launch-kalender|Launch-Kalender KI-Kurs]]
- [[../produkte/ki-treppe/08-aufgabenplan|Aufgabenplan]]
