...
async def cmd_status(message: Message):
    agents = scanner.scan_all()
    logger.info("cmd_status: %d Agents gefunden, sende an chat_id=%s", len(agents), message.chat.id)
    ...
    try:
        ...
        logger.info("cmd_status: gesendet OK, message_id=%s", sent.message_id)
    except Exception as e:
        logger.exception("cmd_status: SENDEN FEHLGESCHLAGEN: %s", e)
