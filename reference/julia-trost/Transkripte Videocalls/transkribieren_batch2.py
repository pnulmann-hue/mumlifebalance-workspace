...
if __name__ == "__main__":
    ...
    print(f"=== Julia Trost Transkription (Batch 2 - Whisper API) ===")
    ...
    for i, url in enumerate(URLS, 1):
        ...
        actual_file = audio_pfad
        if not os.path.exists(actual_file):
            actual_file = audio_pfad + ".mp3"
        if not os.path.exists(actual_file):
            for f in os.listdir(AUSGABE):
                if f.startswith(f"temp_{video_id}") and f.endswith(".mp3"):
                    actual_file = os.path.join(AUSGABE, f)
                    break
